...
const iInputText = document.querySelector('#i-model-path-text')
const pInputText = document.querySelector('#p-model-path-text')

// Restore previous settings
if(localStorage.getItem('iModelPath')) iInputText.value = localStorage.getItem('iModelPath')
if(localStorage.getItem('pModelPath')) pInputText.value = localStorage.getItem('pModelPath')

document.querySelector('#i-model-file').addEventListener('change', (e) => {
  if (e.target.files.length) iInputText.value = e.target.files[0].path || '';
})
document.querySelector('#p-model-file').addEventListener('change', (e) => {
  if (e.target.files.length) pInputText.value = e.target.files[0].path || '';
})

document.querySelector('#btn-show-settings').addEventListener('click', () => {
...
async function startViewerPlayback(targetTopic, isNeural) {
  ...
  if (isNeural) {
    ...
    // Load absolute model paths from settings
    const iPath = localStorage.getItem('iModelPath')
    const pPath = localStorage.getItem('pModelPath')
    
    if(!iPath || !pPath) {
      alert("Missing AI models! Please go to ⚙ AI Settings and select the pretrained .pth.tar files.");
      document.querySelector('#loading-video').classList.add('hidden')
      return;
    }
